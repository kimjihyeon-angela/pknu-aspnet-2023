    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            builder.Services.AddDbContext<ApplicationDbContext>(options => options.UseMySql(
                // appsettings.json ConnectionStrings내의 연결 문자열 할당
                builder.Configuration.GetConnectionString("DefaultConnection"),
                // 연결 문자열로 DB의 서버 버전을 자동으로 가져오는 것을 의미함
                ServerVersion.AutoDetect(builder.Configuration.GetConnectionString("DefaultConnection"))
                ));

            // 2. ASP.Net Identity - ASP.Net 계정을 위한 서비스 설정 (추가)
            ...
            builder.Services.Configure<IdentityOptions>(options =>
            {
                ...
                options.Password.RequiredLength = 4;             // 최소 글자 길이 수
                options.Password.RequireNonAlphanumeric = false; // 특수문자 필요 여부
                options.Password.RequiredUniqueChars = 0;        // 암호 고유문자 수
            });
